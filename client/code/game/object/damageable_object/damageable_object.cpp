...
ref<DamageableObject> DamageableObject::CREATE(Transform* transform, const std::string& model_name, const std::string& pfx_name)
{
	...
	map.insert<object_base_map::String>(0xea402acf, R"(building_blocks\general\oil_barrel_red.lod)");
	map.insert<object_base_map::String>(0x5b982501, R"(models\building_blocks\general\oil_barrel.pfx)");
	map.insert<object_base_map::String>(0xb8fbd88e, R"(Damageable_Barrel)");
	map.insert<object_base_map::Mat4>(0xacaefb1, transform->get_matrix());
